...
export default class AtcService {
  static getAtcUrl(atcProduct, runAll = false) {
    ...
    let url = `https://supremenewyork.com/shop/all/${category}?atc-id=${atcProduct.id}`;
    if (runAll) {
  ...
  static async getAtcMonitorUrl(atcProduct, runAll = false) {
    ...
    if (bestMatch) {
      let url = `https://supremenewyork.com/shop/${bestMatch.id}?atc-color=${atcColor}&atc-id=${atcProduct.id}&atc-monitor=true`;
      if (runAll) {
        url = `${url}&atc-run-all=true`;
      }
      return url;
