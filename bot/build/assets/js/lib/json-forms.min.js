    ...
    renderers["string"] = function (container, id, parentObject, propertyProvider, value) {
      ...
      if (s.type === "any") {
        ...
        for (var i = 0; i < s.enum.length; i++) {
          var enVal = s.enum[i];
          var option = document.createElement("option");
          var isObj = typeof enVal === 'object';
          var optValue = isObj ? enVal.value : enVal;
          var optText = isObj ? enVal.text : enVal;
          var textNode = document.createTextNode(optText);
...
//        }
      ...
      if (r && !s.dependsOn) {
        ...
        if (!value) {
          if (typeof initialValue !== "undefined" && initialValue !== null) {
            value = getInitialValue(id, initialValue);
          } else {
            value = s.default;
          }
        }
    ...
    function getInitialValue(id, initialValue) {
      var ret;
      var key = id.substring(2);
      var val = initialValue[key];
      if (val === undefined) {
        return null;
      }
      return val;
